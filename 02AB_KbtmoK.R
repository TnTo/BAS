...
if (any(abs(rowSums(KC[2:TMAX, ]) * pK[2:TMAX] + rowSums(KK[2:TMAX, ]) * pK[2:TMAX] - K[2:TMAX] * pK[2:TMAX]) > tol)) {
...
if (any(abs(MFC[2:TMAX, ] + sweep(KC[2:TMAX, ], 1, pK[2:TMAX], "*") - VFC[2:TMAX, ]) > tol)) {
    print("FC column in BS not consistent (checked at agent level)")
}
if (any(abs(MFK[2:TMAX, ] + sweep(KK[2:TMAX, ], 1, pK[2:TMAX], "*") - VFK[2:TMAX, ]) > tol)) {
...
if (any(abs(K[2:TMAX] * pK[2:TMAX] - V[2:TMAX]) > tol)) {
