...
ggarrange(
    data %>%
        filter(name %in% c("C", "G", "Y", "I", "W", "P", "GDP")) %>%
        ggplot(aes(x = period, y = value)) +
        geom_line(aes(linetype = name, color = name)),
    data %>%
        filter(name %in% c("C.C", "G.C", "Y.C", "I.C", "W.C", "P.C", "GDP.C")) %>%
        ggplot(aes(x = period, y = value)) +
        geom_line(aes(linetype = name, color = name)),
    data %>%
        filter(name %in% c("C.K", "G.K", "Y.K", "I.K", "W.K", "P.K", "GDP.K")) %>%
        ggplot(aes(x = period, y = value)) +
        geom_line(aes(linetype = name, color = name)),
    nrow = 3
)
...
data %>%
    filter(name %in% c("NC", "NK", "NC.C", "NK.C", "NC.K", "NK.K")) %>%
    ggplot(aes(x = period, y = value)) +
    geom_line(aes(linetype = name, color = name))
...
###
# betaK VERY Low add instability to the model without changing the qualitative dynamics
###
