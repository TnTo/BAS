...
model_eqs <- sfcr_set(
    ...
    NT ~ NCT + NKT,
    N ~ min(1, NT),
    NC ~ min(NCT * N / NT, (NCT * W0FC) / (NCT * W0FC + ICT * pK) * min(NCT * W0FC + ICT * pK, max(0, VFC[-1])) / W0FC),
    NK ~ min(NKT * N / NT, min(NKT * W0FK, max(0, VFK[-1])) / W0FK),
    ...
    G ~ min(Y, GT),
    I ~ NK * betaK,
    IC ~ ifelse(ICT > 0, min(ICT * I / IT, (ICT * pK) / (NCT * W0FC + ICT * pK) * min(NCT * W0FC + ICT * pK, max(0, VFC[-1])) / W0FC), 0),
    IK ~ I - IC,
    P ~ PFC + PFK + PB,
...
model_ext <- sfcr_set(
    ...
    sigma ~ 0.27 # max govt consumption
)
...
model %>%
    pivot_longer(cols = -period) %>%
    filter(name %in% c("I", "IT", "IC", "ICT", "IK", "IKT")) %>%
    ggplot(aes(x = period, y = value)) +
    geom_line(aes(linetype = name, color = name))
...
model %>%
    pivot_longer(cols = -period) %>%
    filter(name %in% c("DL", "VB", "B", "M", "PB", "L", "pKK")) %>%
    ggplot(aes(x = period, y = value)) +
    geom_line(aes(linetype = name, color = name))
