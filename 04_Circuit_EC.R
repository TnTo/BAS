...
model_eqs <- sfcr_set(
    ...
    NT ~ NCT + NKT,
    N ~ min(1, NT),
    NC ~ min(NCT * N / NT, (NCT * W0) / (NCT * W0 + ICT * pK) * min(NCT * W0 + ICT * pK, max(0, VFC[-1])) / W0),
    NK ~ min(NKT * N / NT, min(NKT * W0, max(0, VFK[-1])) / W0),
    W ~ WFC + WFK,
    WFC ~ W0 * NC,
    WFK ~ W0 * NK,
    Y ~ NC * betaC,
    ...
    G ~ min(Y, GT),
    I ~ NK * betaK,
    IC ~ ifelse(ICT > 0, min(ICT * I / IT, (ICT * pK) / (NCT * W0 + ICT * pK) * min(NCT * W0 + ICT * pK, max(0, VFC[-1])) / W0), 0),
    IK ~ I - IC,
    P ~ PFC + PFK + PB,
