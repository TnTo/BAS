...
tgts = 2
# ay av betaC betaK
bounds = [[0.5, 0.1, 0.0, 0.0], [1.0, 0.5, 5.0, 1.0]]
bounds_step = [0.0001] * 4
...
def model(p, n, seed):
    ...
    m = MyModel(T)
    m.set_ext(ay=p[0], av=p[1], aBetaC=p[2], mu=p[3])
    ...
    # res[:, 0] = m.pC[bi:T] / m.pC[(bi - 1) : (T - 1)] - 1
...
cal = MyCalibrator(bounds, bounds_step, target, "cals/03_4p_2t_simplep", model)
