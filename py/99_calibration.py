...
bi = 50
# ay av aBetaC
bounds = [[0.4, 0.05], [1.0, 0.4]]
bounds_step = [0.01, 0.01]
target = np.atleast_2d(np.full((T - bi,), 0.1)).T
# target = np.zeros((T - bi, 1))
# target[:, :] = np.array(
#     [
#         [
#             # 0.03,  # growth rate
...
def model(p, n, seed):
    ...
    res = np.atleast_2d(1.0 - (m.NC[bi:T] + m.NK[bi:T])).T

    return res


cal = MyCalibrator(bounds, bounds_step, target, "2_vars", model)
