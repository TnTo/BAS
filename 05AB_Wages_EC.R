...

{
    ...
    ## Constants
    {
        ...
        sigma <- 0.2 # target G consumption fraction
    }
    ...
    ## A model
    {
        ...
        for (t in 2:TMAX) {
            ...
            GT[t] <- sigma * NH * betaC # Desired Demand for Gvt, in units of goods
            ...
            C[t, , ] <- C[t - 1, , ]
            # Over-selling
            Fids <- which(colSums(C[t, , ]) - (Y[t, ] - G[t, ]) > tol)
            if (length(Fids) > 0) {
                for (Fid in Fids) {
                    while (sum(C[t, , Fid]) - (Y[t, Fid] - G[t, Fid]) > tol) {
                        Hid <- sample.vec(which(C[t, , Fid] > 0), 1)
                        d <- min(C[t, Hid, Fid], sum(C[t, , Fid]) - (Y[t, Fid] - G[t, Fid]))
            ...
            repeat {
                ...
                if (length(Hids > 0) && (sum(Y[t, ] - G[t, ]) - sum(C[t, , ]) > tol)) {
                    Hid <- sample.vec(Hids, 1)
                    Fids <- which(Y[t, ] - G[t, ] - colSums(C[t, , ]) > 0)
                    Fid <- Fids[which.min(pC[t, Fids])]
                    d <- min(CT[t, Hid] - sum(C[t, Hid, ]), ((DI[t, Hid] + MH[t - 1, Hid]) - sum(C[t, Hid, ] * pC[t, ])) / pC[t, Fid], Y[t, Fid] - G[t, Fid] - sum(C[t, , Fid]))
                    C[t, Hid, Fid] <- C[t, Hid, Fid] + d
                } else {
                    break
                }
