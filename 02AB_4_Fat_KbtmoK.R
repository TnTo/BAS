...

{
    ...
    # Initial values
    {
        MH[1, ] <- rlnorm(NH) # Initialize with flat wealth distribution
        MFC[1, ] <- 0
        MFK[1, ] <- 0
        M[1] <- sum(MH[1, ])
        DI[1, ] <- 0
    ...
    ## A model
    {
        ...
        for (t in 2:TMAX) {
            ...
            PFK[t, , ] <- sweep(t(array((pK[t] * colSums(IC[t, , ]) - colSums(WFK[t, , ])), c(NFK, NH))), 1, MH[t - 1, ], "*") / sum(MH[t - 1, ])
            PFC[t, , ] <- sweep(t(array((pC[t] * S[t, ] - pK[t] * rowSums(IC[t, , ]) - colSums(WFC[t, , ])), c(NFC, NH))), 1, MH[t - 1, ], "*") / sum(MH[t - 1, ])
