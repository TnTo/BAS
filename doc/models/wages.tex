...
Additionally, an AB version of the model with constrained government spending and credit lines has been made.
...
\section{An Object-Oriented version}
Finally, this last version of the model with endogenous spending has been rewritten following an object-oriented logic as detailed in chapter \ref{ch:implementation}.
The code is reported in the appendix and it's written in python rather than in R.

The variables are gathered by agent, with each agent collecting all the flows, stocks and other variables relevant to them.

The order of the events is almost identical but the stock of each agent are updated continuosly and agents remains the same through the time-steps, while in the matrix-based approach the stock are computed only at the end and each time-step corresponds to a different column.

Additionally, each capital good is now represented by a different object, allowing heterogeneity among them and a more precise computation of the depreciation.

This approach highlight the double and quadruple entries principle: all the stocks and the flows must be update in both the agents involved, which results in two lines of code each updating the information for one of the two.
Moreover, each flow requires the update of two stocks, one for each agent involved.

The dynamic of the model is very similar, with some little tweaks on the initial conditions to stabilize the model.