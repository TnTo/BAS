...
The goal of this extension is to add a stylezed financial sector inspered to Graziani circuit theory \parencite{passarella2024, caiani2017}: firms get loans from the financial sector to pay for wages and investments and repay them at the end of the period.
...
The matrices of the models change as below: money is now issued by the bank as deposits and backed by government bonds (which can serve as reserves if the sign switches), while firms have access to credit trough the loans issued by the banking sector.
Both loans and bonds are interests bearing.

\section{The aggregate version}
\subsection{The behavioural equations}
Firms receive loanf from the bank to cover for wages and investment $$\Delta^+ \mathbf{L}_{\mathcal{F}_\mathbf{C}} = W_{\mathcal{F}_\mathbf{C}} + p_\mathbf{K} I_{\mathcal{F}_\mathbf{C}}$$ $$\Delta^+\mathbf{L}_{\mathcal{F}_\mathbf{K}} = W_{\mathcal{F}_\mathbf{K}}$$
...
Then interest are paied on maximum amount of loans inside the period $\mathbf{L}_{t-1} + \Delta^+\mathbf{L}$

Bonds are issued by the government to finance the deficit and interests are paied on them; the accounting at the end of the period is $$\mathbf{B} = \frac{\mathbf{B}_{t-1} - (T - p_\mathbf{C}G - UB)}{1-r_\mathbf{B}}$$
...
The profits are mostly concentrated in the consumption goods sectore (figure \ref{fig:04_P}) and the circuit closes entirely, not leaving a stock of loans at the end of the period (figure \ref{fig:04_L}).
...
This is clearly an effect of the endogeineity of money, which is now created without limits by the financial sector, being able to accomodate financially any growth of demand.
...
We impose as the restriction on the credit lines for each sector that $$\Delta^+\mathbf{L} \le V_{t-1} + \mathbf{M}_{t-1}$$ representing the necessity of a collateral good required by the bank.

Already in the previous versions the markup rate of the capital sector decline until it reach zeros, but since all the costs are considered in the determination of the price of the capital goods, firms are able to operate without issues.
This version add the interest on the loans taken to start the circuit as a cost of production, which is not taken in account in determining the prices.
It is possible to either adjust the price equation or to fix a minimum value for the markup. The second option has been choosen.
...
For realism, financial sector can be populated by competiting banks, financial firms that manage savings and trade in bonds and equities.
...
To tame the complexity of the model we choose to considered the financial sector as a very stylized aggregate sector rather than a detailed sector with multiple agents.

The agent based follows closely the aggegate version with endogenous government spending, including the absence of constrains on credit.
The only relevant difference is a consequence of the heterogeinity of the firms: as a consequence of the random matching, some firms accomulate enough debt to reach negative wealth.
...
\begin{figure}
    ...
    \caption{Dynamics of the AB model with the circuit}
    \label{fig:04AB_L}
\end{figure}

In figure \ref{fig:04AB_L} we can observe the spike in the stock of loans held by capital firms before the bankrupticies. Additionally profits are negligible in the capital firms sector and are distributed mostly by the banking one and the consumption firms one.

Additionally the fat-tail distribution of wealth among households becomes even more skewed, resembling a one-gets-all dynamics.
