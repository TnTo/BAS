...
The OOP version is significantly slower, because the agents are simulated one-by-one, rather than all toghether as a single row in the matrix.
This will allow for more complex behavioural equations and interactions at the cost, as said, of increased computational time and resources.

Another difference to be noted is that the matrix-based approach allows for updating the stocks at the and of the period, while the OOP version updates them continously.
This highlights the double and quadruple entries principle: all the stocks and the flows must be update in both the agents involved, which results in two lines of code each updating the information for one of the two.
