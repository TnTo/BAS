...
sh1 <- sfcr_shock(
    variables = sfcr_set(ay ~ 0.8, av ~ 0.2),
    start = 1,
    end = L
)
...
sh2 <- sfcr_shock(
    variables = sfcr_set(ay ~ 0.6, av ~ 0.4),
    start = 1,
    end = L
)
...
sh4 <- sfcr_shock(
    variables = sfcr_set(ay ~ 0.7, av ~ 0.4),
    start = 1,
    end = L
)
...
data <- (m0 %>% rename_with(~ paste(., ".6.2", sep = ""), -period)) %>%
    full_join(m1 %>% rename_with(~ paste(., ".8.2", sep = ""), -period), by = "period") %>%
    full_join(m2 %>% rename_with(~ paste(., ".6.4", sep = ""), -period), by = "period") %>%
    full_join(m4 %>% rename_with(~ paste(., ".7.4", sep = ""), -period), by = "period") %>%
    pivot_longer(cols = -period)

suffixes <- c(".6.2", ".8.2", ".6.4", ".7.4")
