...
class MyModel:
    ...
    def step(self, t):
        ...
        for _ in range(50):
            ...
            self.N[t] = min(1, self.YT[t] / (self.beta * self.p[t - 1]))
            self.p[t] = (1 + self.mu) * self.W[t] / (self.N[t] * self.beta)
            self.Y[t] = self.N[t] * self.beta * self.p[t]
